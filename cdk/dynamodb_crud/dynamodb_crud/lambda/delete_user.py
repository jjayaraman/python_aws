...
def lambda_handler(event, context):
    ...
    try:
        ...
        response = delete(user_id, timestamp)
        if response is None:
            message = f"No user exists for the given user id : {user_id} and timestamp: {timestamp}"
            status_code = 400
        else:
            message = f"User {user_id} with timestamp: {timestamp} deleted successfully"
            status_code = 200
